...
const mockMiddleware = async (ctx, next) => {
    ...
    let base = JSONC.parse(fs.readFileSync(path.join(mockPath, '__base.json'), 'utf8'));
    let data = JSONC.parse(fs.readFileSync(path.join(mockPath, file_path), 'utf8'));
...
module.exports = (targetUrl) => {
    ...
    console.log(chalk.green(`mock server start on port: ${mockPort}`));

    return app.listen(mockPort);
};
